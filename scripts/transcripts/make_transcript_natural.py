...
def main() -> int:
    in_path = Path(sys.argv[1]) if len(sys.argv) >= 2 else Path("data/transcripts/transcribe.txt")
    out_path = Path(sys.argv[2]) if len(sys.argv) >= 3 else Path("data/transcripts/transcribe_natural.txt")
